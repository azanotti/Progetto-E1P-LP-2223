...
; at result associates the member, if what is associated with other_objects is
; a '}' then the object is (jsonobj result).
...
; matches the string, and if the head of the one mapped to other_pairs matches
; ':' then reuse the multiple-value-bind macro which use identifyValue and at
; result_other_pairs associates the pair.
  ...
  (cond ((or (eql (car number) #\-)
         ...
         (multiple-value-bind (result other_numbers)
           ...
           (values (car (multiple-value-list
                         (read-from-string
                          (coerce
                           (cons (car number) result) 'string)))) other_numbers)))
        (T (error "ERROR: syntax error"))))
...
(defun lispArrayToJsonString (j_array jsonstring)
  (cond ((and (null j_array) (eq (char jsonstring 0) #\[))
	...
	((eq (car j_array) 'jsonarray)
	 (lispArrayToJsonString (cdr j_array) (concatenate 'string jsonstring "[")))
